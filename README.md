...
검색 API의 캐시 적용 전/후 성능 차이를 확인하기 위해 k6를 이용한 부하 테스트를 수행했습니다.

### 테스트 대상
- v1: `/api/v1/products?keyword=스타벅스&page=0&size=10`
- v2: `/api/v2/products?keyword=스타벅스&page=0&size=10`

### 테스트 환경
- MySQL
- Redis Remote Cache
- 검색 더미 데이터 50,000건 이상
- k6 `v2.0.0-rc1`

### 결과 요약
| 구분 | avg 응답시간 | p95 | TPS |
| --- | ---: | ---: | ---: |
| v1 기본 | 114.8ms | 161.76ms | 22.20 |
| v2 기본 | 3.13ms | 3.73ms | 24.68 |
| v1 고부하 | 786.16ms | 1.55s | 46.89 |
| v2 고부하 | 2.92ms | 3.76ms | 83.19 |

### 결론
Redis Cache 적용 후 검색 API의 응답 속도가 크게 개선되었고, 고부하 상황에서도 더 높은 처리량과 안정성을 유지하는 것을 확인했습니다.