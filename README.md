...
- 로그인한 사용자가 본인이 등록한 판매 상품 목록을 조회하는 API입니다.

### 대상 선정 이유

- 해당 API는 판매자가 마이페이지 또는 판매내역 화면에서 자주 호출할 수 있는 조회 기능입니다.  
- 상품 데이터가 많아질수록 특정 판매자의 상품을 찾기 위한 `seller_id` 조건 검색과 판매 상태를 필터링하는 `sale_status` 조건 검색이 반복적으로 발생합니다.

- 또한 판매 상품 목록은 최신순으로 보여주는 경우가 많기 때문에 `created_at DESC` 정렬이 함께 발생합니다.  
...
| ------ |------| ---- |------:| --------------------------- |
| Before | ref  | idx_gifticon_sale_seller_id | 49901 | Using where; Using filesort |

### 분석(문제점)
- 기존에는 seller_id에 대한 FK 인덱스가 존재하여 type=ref로 조회되었습니다.
- 하지만 created_at DESC 정렬에 사용할 수 있는 복합 인덱스가 없어 Extra에 Using filesort가 발생했습니다.
- 테스트 데이터 대부분이 `seller_id = 1`과 `sale_status = 'ON_SALE'` 조건을 만족하도록 구성되어 있어, Before/After의 rows 값은 동일하게 나타났습니다.
...
### 인덱스 설계 이유
- seller_id는 특정 판매자의 상품을 조회하기 위한 조건 컬럼이고, sale_status는 판매 상태를 필터링하는 조건 컬럼입니다.
- created_at은 최신순 정렬에 사용됩니다.
- 따라서 seller_id, sale_status, created_at DESC 순서의 복합 인덱스를 생성하여 WHERE 조건과 ORDER BY 정렬을 함께 최적화하도록 설계했습니다.
- 이를 통해 특정 판매자의 판매중 상품을 찾고, 그 결과를 최신순으로 정렬하는 과정까지 하나의 복합 인덱스로 처리할 수 있도록 설계했습니다.
...
| 구분    | type | key  |  rows | Extra                       |
|-------|------| ---- |------:| --------------------------- |
| After | ref  | idx_gifticon_sale_seller_status_created_at | 49901 |Using index condition |


- 복합 인덱스 적용 후 기존 seller_id 단일 FK 인덱스 대신 idx_gifticon_sale_seller_status_created_at 인덱스가 사용되었습니다.
- 이를 통해 seller_id, sale_status 조건 검색과 created_at DESC 정렬을 함께 처리할 수 있게 되었고, 기존에 발생하던 Using filesort를 제거하거나 정렬 비용을 줄일 수 있었습니다.
...
|---|---:|---|------:|-------:|---|
| Before | 50,000건 | idx_gifticon_sale_seller_id | 49901 |  993ms | Using filesort |
| After | 50,000건 | idx_gifticon_sale_seller_status_created_at |   49901 | 743 ms | Using index condition |
...
- INSERT, UPDATE, DELETE가 발생할 때마다 테이블 데이터뿐만 아니라 인덱스도 함께 갱신되어야 하므로 쓰기 성능에는 부담이 생길 수 있습니다.
